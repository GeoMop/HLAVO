**Plan**
- Locate project structure under `/workspace` and confirm where to place code (likely `deep_model/gpt_zone`), plus the read-only GIS directory and `uhelna_all.qgz`.
- Define YAML config schema (meshsteps, model dir, conductivities, modflow settings).
- Implement GIS readers:
  - Load `JB_extended_domain` polygon.
  - Load `HG model layers` rasters in surface->bottom order.

- Establish local coordinate origin (first polygon point rounded to 1000 m) and transform all XY to local coords.
- Build grid:
  ...
  - Assign materials bottom-up using `last_top` per column; refine Z to match layer boundaries and even spacing within each layer.
