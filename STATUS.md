...
- Tracked/uncommitted: [hlavo/main.py](/home/hlavo/workspace/hlavo/main.py) and [hlavo/composed/worker_1d.py](/home/hlavo/workspace/hlavo/composed/worker_1d.py) now keep root logging at INFO and enable DEBUG only for `hlavo.*`. Worker file logs are written to per-site files such as `worker_1d_site_1.log`, not to the main `calculation.log`; the worker-specific `contextvars` routing was later simplified away and each 1D worker now adds its file handler directly to the `hlavo` logger tree.
- Tracked/uncommitted: [hlavo/misc/logging_utils.py](/home/hlavo/workspace/hlavo/misc/logging_utils.py) now owns the reusable file-handler helper for both root-level run logs and named-logger HLAVO-only logs. `ensure_file_handler(...)` documents the logging behavior and supports `logger_name="hlavo"`; [hlavo/composed/worker_1d.py](/home/hlavo/workspace/hlavo/composed/worker_1d.py) uses that shared helper instead of a local `_ensure_worker_file_handler(...)`.
- Tracked/uncommitted: [hlavo/misc/logging_utils.py](/home/hlavo/workspace/hlavo/misc/logging_utils.py) now exposes explicit logger-object based APIs: `set_hlavo_loggers(...)`, `ensure_stdout_handler(logger, ...)`, and `ensure_file_handler(logger, ...)`. The wrapper `calculation_formatter()` was removed in favor of direct `LevelFormatter()` usage, and [hlavo/main.py](/home/hlavo/workspace/hlavo/main.py) plus [hlavo/composed/worker_1d.py](/home/hlavo/workspace/hlavo/composed/worker_1d.py) now pass `HLAVO_ROOT_LOGGER` / `HLAVO_DEBUG_LOGGER` explicitly.
- Tracked/uncommitted: [hlavo/misc/logging_utils.py](/home/hlavo/workspace/hlavo/misc/logging_utils.py) was simplified again to match the fixed policy: `ensure_stdout_handler(...)` always attaches to the root logger and `ensure_debug_file_handler(...)` always attaches to the `hlavo` logger tree. [hlavo/main.py](/home/hlavo/workspace/hlavo/main.py) and [hlavo/composed/worker_1d.py](/home/hlavo/workspace/hlavo/composed/worker_1d.py) now use that narrower API.
- Tracked/uncommitted: [hlavo/soil_parflow/parflow_model.py](/home/hlavo/workspace/hlavo/soil_parflow/parflow_model.py) now writes explicit `start:` / `finish:` markers to `parflow.log`, and converts ParFlow face-centered vertical velocity (`NZ + 1`) to a cell-centered `NZ` profile in `get_data(..., "velocity")` so the ParFlow wrapper matches the Kalman/test expectations.
...
- Tracked/uncommitted: [hlavo/composed/worker_1d.py](/home/hlavo/workspace/hlavo/composed/worker_1d.py) configures Dask worker logging to stdout and the shared workdir `calculation.log`.
...
- Untracked: [hlavo/misc/logging_utils.py](/home/hlavo/workspace/hlavo/misc/logging_utils.py) provides a shared formatter: compact INFO lines and source-aware DEBUG lines.
...
- `python3 -m py_compile hlavo/soil_parflow/parflow_model.py` passed after restoring explicit `parflow.log` run markers and normalizing vertical velocity shape.
- `PYTEST_ADDOPTS='tests/soil_parflow/test_parflow_model.py -q' timeout 180s tests/run` passed after normalizing ParFlow `velz` to a cell-centered profile: `3 passed`.
...
- The requested pre-write staging step could not be completed in this session because `.git/index.lock` already existed and `git add hlavo/composed/worker_1d.py hlavo/misc/logging_utils.py PLAN.md STATUS.md` failed before the edits started.
- The actual Python root logger was not renamed, and the repository still logs through `hlavo.*`. This refactor only made the logger objects explicit at the helper API level; a full move to a distinct `hlavo_debug.*` logger tree would require a repo-wide logger-name migration.
